...
class FramelessWindow(QMainWindow):
    def __init__(self):
        ...
        self.title_label.setStyleSheet(
            "font-weight: 500; font-size: 13px; padding-left: 15px;"
        )
        ...
        self.status_bar.showMessage(
            "  Hệ thống chạy trên nền cấu hình Material Design."
        )
    ...
    def update_window_styles(self, is_maximized):
        ...
        radius = (
            "0px" if is_maximized else "16px"
        )  # Material 3 sử dụng góc bo lớn từ 12-24px

        # Cập nhật viền cho Widget tổng, thanh tiêu đề và thanh status bar
        self.central_widget.setStyleSheet(
            f"QWidget#CentralWidget {{ border-radius: {radius}; }}"
        )
        if hasattr(self, "title_bar") and hasattr(self, "status_bar"):
            self.title_bar.setStyleSheet(
                self.title_bar.styleSheet()
                + f" QWidget#TitleBar {{ border-top-left-radius: {radius}; border-top-right-radius: {radius}; }}"
            )
            self.status_bar.setStyleSheet(
                f"QStatusBar#StatusBar {{ border-bottom-left-radius: {radius}; border-bottom-right-radius: {radius}; }}"
            )
